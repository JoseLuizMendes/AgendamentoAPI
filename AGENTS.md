...
> (`Dev/AGENTS.md`), adaptadas para um repositório de código.
>
> **Boot obrigatório:** antes de qualquer trabalho substantivo, leia (1) este arquivo e
> (2) o `AGENTS.md` da subpasta em que você vai mexer (`api/`, `api/src/services/`,
...
> isolados em `specs/NNN-*/`. Este `AGENTS.md` e os por-pasta seguem como **contexto operacional
...
> `/speckit-implement` (skills em `.Codex/skills/speckit-*`).
...
- **C4 — Fonte da verdade das regras.** São este `AGENTS.md` + o `Preferencias Dev`. Em
...
2. Ler o `AGENTS.md` da subpasta-alvo.
