...
- **Client errors** (`POST /client-errors`): público (allowlist em `plugins/auth.ts`), rate limit 30/min/IP; valida (Zod) e loga via pino (`req.log.error`), **sem banco**; responde 204. Observabilidade caseira (`specs/001-…`).
...
- **Observabilidade de frontend (concluído, `specs/001-…`):** Error Boundaries (`app/error.tsx` +
  `app/global-error.tsx`, fallback sem tela branca), listeners globais (`window` error/rejection) e
  reporter fire-and-forget (`lib/report-error.ts`) → `POST /client-errors` (loga com `appVersion`).
  Sem dependência nova. Testes: `api/tests/unit/client-errors.test.ts` + `web/src/lib/report-error.test.ts`.
...
- **Observabilidade de frontend** (`specs/001-…`): **implementada** (US1 fallback + US2 reporte/endpoint).
  Adiado para fatia futura: Web Vitals e persistência/dashboard de erros (hoje só log na VPS).
