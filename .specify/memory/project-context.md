...
- **Error Boundaries (UX):** `app/error.tsx` + `app/global-error.tsx` (fallback sem tela branca,
  `reset()`). _O reporte caseiro de erro (`POST /client-errors` + `lib/report-error.ts` + listeners)
  foi **removido** — observabilidade de erro vira serviço externo (Sentry/Datadog/Grafana)._
...
- **Observabilidade:** Error Boundaries mantidas (UX). O reporte/telemetria de erro será **serviço
  externo** (Sentry/Datadog/Grafana) — o caseiro `POST /client-errors` foi removido (não usar caseiro).
