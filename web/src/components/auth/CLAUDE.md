...
- Submit de login/signup chama `apiRequest` e grava o token via `lib/auth` (`setToken`), depois
  navega para `/{slug}`. (Auth não gerencia cache de servidor → não precisa de React Query.)
