...
function mapAppointment(a: {
  ...
  return {
    ...a,
    status: a.status as "SCHEDULED" | "CANCELED",
  };
}

export function createRepositories(db: PrismaClient | Prisma.TransactionClient): Repositories {
  return {
    services: {
      async list() {
    ...
    businessHours: {
      ...
      async replaceBreaks(dayOfWeek, breaks) {
        ...
        if (!updated) throw new Error("BusinessHours não encontrado");
...
export function isRetryableSerializableError(err: unknown): boolean {
