...
export interface ServiceRepository {
  ...
  create(input: { name: string; priceInCents: number; durationInMinutes: number }): Promise<Service>;
  update(
    id: number,
...
export interface AppointmentRepository {
  ...
  findScheduledConflict(args: { startTime: Date; endTime: Date }): Promise<{ id: number } | null>;
  create(input: {
    customerName: string;
  ...
  cancelOptimistic(args: { id: number; version: number }): Promise<{ updated: boolean }>;
}
